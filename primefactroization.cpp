...
void compute(int a, int b) {
    ...
    map<int, int> combined = mp1; 
    for (const auto& primes : mp2) {
        int prime = primes.first;
        combined[prime] = max(mp1[prime], primes.second);
    }
    for (const auto& primes : combined) {
        lcm *= pow(primes.first, primes.second); 
    }
    cout << gcd << " " << lcm << endl;
}
int main() {
